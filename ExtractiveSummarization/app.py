...
@app.route('/SummaryPost', methods=['POST'])
def test():
    req=request.get_json()
    print(req["text"])
    os.chdir(cwd + "/articles")
    dataToSummarize = req["text"]
    global slider
    slider=''
    slider=req["sliderValue"]
    print(dataToSummarize)
    global username
    username = ''
    username = req["user"]
    global filename
    filename="articel1_"+username
    file = open(filename, "w", encoding="utf8")
    file.write(dataToSummarize)
    file.close()
    t=''
    os.chdir(cwd)
    getSummary()
...
def welcome():
    os.chdir(cwd + "/articles")
    dataToSummarize = request.args.get('text')
    print(dataToSummarize)
    global username
    username = ''
...
def executeForAFile(filename, output_file_name, cwd):
    ...
    temp = rbm.test_rbm(dataset=featureMat_normed, learning_rate=0.1, training_epochs=25, batch_size=5, n_chains=5,
                        n_hidden=8)
    ...
    #print(enhanced_feature_sum)
    ratio = 100 - int(slider)
    length_to_be_extracted  = (len(enhanced_feature_sum) * ratio ) // 100
...
def getSummary():
    ...
    for file in os.listdir(cwd + "/articles"):
        ...
        output_file_list.append(file)
    for x in range(len(filenames)):
        if filenames[x] != 'enhancedFMatrix.csv':
            executeForAFile(filenames[x], output_file_list[x], cwd)
